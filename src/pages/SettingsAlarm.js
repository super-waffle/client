...
export default function SettingsAlarm() {
  ...
  const onClickIsChecked = useCallback(() => {
    axios
      .post(
        process.env.REACT_APP_SERVER_URL + `/notices/${noticeSeq}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${TOKEN}`,
    ...
    <div className="settings-notice">
          ...
          <tbody>
            {notice.noticeData &&
              notice.noticeData.map((alarm) => (
                <tr
                  key={alarm.noticeSeq}
                  className={`"settings-notice-board-row" + ${alarm.isChecked ? 'is-checked' : 'not-checked'}`}
                >
                  <td className={`settings-notice-board-category + ${alarm.categorySeq === 101 ? 'alert' : 'normal'}`}>
                  ...
                  <td className="settings-notice-board-date">{alarm.noticeDate}</td>
                </tr>
              ))}
          </tbody>
        </table>
      ...
      <Modal open={modalOpen} close={closeModal} header="">
        <div className="modal-notice-heading">
          <div className={`modal-notice-category ${noticeModalCategorySeq === 101 ? 'alert' : ''}`}>
            [{noticeModalCategory}]
          </div>
