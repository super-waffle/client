...
export default function StudyRecruit() {
  ...
  const onClickSearch = () => {
    if (isLogin) {
      axios
        .get(process.env.REACT_APP_SERVER_URL + `/studies?page=1&type=${category}&key=${searchInput}`, {
          headers: {
  ...
  useEffect(() => {
    if (isLogin) {
      ...
      axios
        .get(process.env.REACT_APP_SERVER_URL + `/studies?page=${currentPage}&type=${category}&key=${searchInput}`, {
          headers: {
            Authorization: `Bearer ${TOKEN}`,
          },
        })
