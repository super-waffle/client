...
export default function SettingsProfile() {
  ...
  const [prevTimeGoal, setPrevTimeGoal] = useState('');
  ...
  async function getUserInfo() {
    ...
    setPrevTimeGoal(USER.userTimeGoal / 60);
  }
  useEffect(() => {
    if (isLogin()) {
      getUserInfo();
  ...
  const deleteFileImage = () => {
    ...
    setIsActive(!isActive);
    axios
      .patch(process.env.REACT_APP_SERVER_URL + '/users/profile/image', dataWithoutImg, {
        headers: {
          Authorization: `Bearer ${TOKEN}`,
        },
      })
  ...
  // console.log(timeGoal, prevTimeGoal);
      ...
      <div className="settings-profile__box">
        ...
        <div className="settings-profile__box-body">
          <div className="settings-profile__box-text">
            ...
            <div className="settings-profile__box-text__title">목표 공부 시간</div>
            <input
              className="settings-profile__box-text__timegoal"
              defaultValue={prevTimeGoal}
              onChange={(event) => {
            ...
            />
            <button className="settings-profile__box-btn" onClick={onClickUpdate}>
              프로필 업데이트
            </button>
          </div>
          <div className="settings-profile__box-img">
            ...
            <div className="settings-profile__box-img-file-wrapper">
              ...
              {profileImg && !fileImage && <img className="settings-profile__box-img-file" src={imageURL} alt="" />}
              {profileImg && fileImage && <img className="settings-profile__box-img-file" src={fileImage} alt="" />}
            </div>
            <button className="settings-profile-box-img-edit" onClick={onClickDropdown}>
            ...
            <div ref={dropdownRef} className={`profileImageEdit-dropdown ${isActive ? 'active' : 'hidden'}`}>
